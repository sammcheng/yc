...
export default function DeveloperConsole() {
  ...
  const handleSubmit = async () => {
    if (!formData.agentName || !formData.description || !creatorWallet || !pricePerCall) {
    ...
    try {
      const response = await fetch('/api/agents', {
        ...
        body: JSON.stringify({
          ...
          capabilities: formData.tags.length > 0 ? formData.tags : [formData.category],
          ...
          <div className="lg:col-span-2">
              ...
              {activeTab === 'monetization' && (
                ...
                >
                  <section>
                    ...
                    <p className="text-gray-500 dark:text-gray-400 mb-6">Set up pricing and configure your wallet to receive payments.</p>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        <label className="flex flex-col">
                          <p className="text-base font-medium leading-normal pb-2">Creator Wallet Address *</p>
                          <input 
                            type="text"
                            value={creatorWallet}
                      ...
                      <div>
                        <label className="flex flex-col">
                          <p className="text-base font-medium leading-normal pb-2">Price Per Call (USD) *</p>
                          <input 
                            type="number"
                            step="0.01"
                            value={pricePerCall}
                      ...
                      <div className="md:col-span-2">
                        <label className="flex flex-col">
                          <p className="text-base font-medium leading-normal pb-2">API Endpoint (Optional)</p>
                          <input 
                            type="text"
                            value={apiEndpoint}
                      ...
                      <div className="md:col-span-2">
                        <label className="flex flex-col">
                          <p className="text-base font-medium leading-normal pb-2">API Key (Optional)</p>
                          <input 
                            type="password"
                            value={apiKey}
                    ...
                    {submitSuccess && (
                      <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4 mt-4">
                        <p className="text-green-600 dark:text-green-400 text-sm">Agent created successfully!</p>
                      </div>
                    )}
                  </section>
                </motion.div>
            ...
            >
              ...
              <motion.button 
                ...
                disabled={submitting}
                className="flex min-w-[84px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-10 px-4 bg-custom-blue text-white text-sm font-bold hover:bg-blue-600 transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed"
              >
                <span className="truncate">
                  {submitting ? 'Submitting...' : activeTab === 'monetization' ? 'Submit for Review' : `Next: ${tabs.find(t => t.id === (activeTab === 'profile' ? 'capabilities' : 'monetization'))?.label}`}
                </span>
              </motion.button>
            </motion.div>
          </div>
